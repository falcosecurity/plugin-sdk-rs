...
macro_rules! ensure_plugin_capabilities {
    ($ty:ty) => {
        const _: () = {
            ...
            let impls_async =
                $crate::async_event::wrappers::AsyncPluginApi::<$ty>::IMPLEMENTS_ASYNC;
            let impls_extract =
                $crate::extract::wrappers::ExtractPluginApi::<$ty>::IMPLEMENTS_EXTRACT;
            let impls_listen =
                $crate::listen::wrappers::CaptureListenApi::<$ty>::IMPLEMENTS_LISTEN;
            let impls_parse =
                $crate::parse::wrappers::ParsePluginApi::<$ty>::IMPLEMENTS_PARSE;
            let impls_source =
                $crate::source::wrappers::SourcePluginApi::<$ty>::IMPLEMENTS_SOURCE;
